...
export default function ContractDefinitionLoader (i: { web3: any, contractDefinitions: IContractDefinitions, options: any }) {
  ...
  for (let key in contractDefinitions) {
    ...
    const { contract_name, contractName, networks, abi, unlinked_binary } = contractDefinition;
    const network = _.first(_.reverse(_.sortBy(networks, "updated_at")));
    let address: (string|null) = null;
    if (network) {
      console.info(`Creating contract ${contract_name || contractName} pegged to ${network.address}.`);
      address = network.address;
    } else {
      console.info(`Creating contract ${contract_name || contractName} unpegged.`);
    }

    if (semver.satisfies(web3.version.api ? web3.version.api : web3.version, "< 1.0.0")) {
      contracts[contract_name || contractName] = load_pre_100(web3, abi, address, options);
      contracts[contract_name || contractName].options.data = unlinked_binary;
    } else {
      contracts[contract_name || contractName] = load_100(web3, abi, address, options);
      contracts[contract_name || contractName].options.data = unlinked_binary;
    }
  }
